...
async function seed(): Promise<void> {
  ...
  for (let i = 30; i >= 1; i--) {
    ...
    summaryValues.push(
      `(1, '${date}', ${(parseFloat(gen) * 24).toFixed(2)}, ${gen}, ${(parseFloat(gen) * 0.8).toFixed(2)}, ${(parseFloat(gen) * 0.4).toFixed(2)}, ${freq}, ${(parseFloat(freq) - 0.05).toFixed(3)}, ${(parseFloat(freq) + 0.05).toFixed(3)}, ${volt}, ${(parseFloat(volt) - 10).toFixed(2)}, ${(parseFloat(volt) + 10).toFixed(2)}, ${pf}, ${wl}, ${(parseFloat(wl) - 0.5).toFixed(3)}, ${down}, ${Math.floor(Math.random() * 5)})`
    );
  }
  ...
  await db.query(`
    ...
    VALUES
      ...
      ('Generator.Temperature','HIGH_WARNING', 'warning',  'resolved', 'Temperature HIGH WARNING: 82 deg C (threshold: 80)', 82,  80,   NOW() - INTERVAL '1 day',  NOW() - INTERVAL '1 day'  + INTERVAL '30 minutes')
