...
const COLORS: string[] = ['#3b82f6','#10b981','#f59e0b','#ef4444','#8b5cf6','#06b6d4','#f97316'];

/** Merged row keyed by bucket; keys are tag names with string values (4 decimal places) */
type MergedRow = { time: string } & Record<string, string>;

export default function Historical() {
  const [selectedTags, setSelectedTags] = useState<string[]>(['Generator.Power', 'Generator.Frequency']);
  const [from, setFrom] = useState<string>(format(subHours(new Date(), 6), "yyyy-MM-dd'T'HH:mm"));
  const [to,   setTo]   = useState<string>(format(new Date(), "yyyy-MM-dd'T'HH:mm"));
  const [bucket, setBucket] = useState<string>('10');

  const enabled = selectedTags.length > 0;

  // NOTE: Calling useQuery inside .map violates the Rules of Hooks.
  // This works at runtime only when `selectedTags` never changes length between renders in a
  // way that reorders calls. Preserved as-is to keep logic identical to the original JS.
  // eslint-disable-next-line react-hooks/rules-of-hooks
  const queries = selectedTags.map((tag) =>
    // eslint-disable-next-line react-hooks/rules-of-hooks
    useQuery<AggregatedReading[]>({
      queryKey: ['history-agg', tag, from, to, bucket],
      queryFn: () =>
        readingsApi.aggregated({
          tagName: tag,
  ...
  const mergedMap: Record<string, MergedRow> = {};
  queries.forEach((q, i) => {
    (q.data ?? []).forEach((point: AggregatedReading) => {
  ...
  const chartData: MergedRow[] = Object.values(mergedMap).sort((a, b) =>
    a.time > b.time ? 1 : -1
  );
      ...
      <div className="card space-y-4">
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <div>
            ...
            <select
              value={bucket}
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setBucket(e.target.value)}
            ...
            <table className="w-full text-xs">
              <thead className="sticky top-0 bg-hydro-card">
                <tr className="border-b border-hydro-border text-slate-400 uppercase tracking-wider">
                  ...
                  {selectedTags.map((t) => (
                    <th key={t} className="px-4 py-3 text-left">{t.split('.')[1]}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
