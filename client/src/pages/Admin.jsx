...
const TABS = ['SCADA', 'Tags', 'Users', 'Notifications', 'System'];
      ...
      <div className="flex gap-2 border-b border-hydro-border pb-3">
        {TABS.map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === t ? 'bg-brand-600 text-white' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
            {t}
          </button>
        ))}
...
// ─── SCADA Settings ───────────────────────────────────────────────────────────
function ScadaSettings({ qc }) {
  const { data } = useQuery({ queryKey: ['scada-settings'], queryFn: () => adminApi.scadaSettings().then((r) => r.data.settings) });
  const [form, setForm] = useState({});

  const mut = useMutation({
    mutationFn: (d) => adminApi.updateScada(d),
    onSuccess: () => { toast.success('Settings saved'); qc.invalidateQueries(['scada-settings']); },
...
// ─── Tag Settings ─────────────────────────────────────────────────────────────
function TagSettings({ qc }) {
  const { data, isLoading } = useQuery({ queryKey: ['tags'], queryFn: () => adminApi.tags().then((r) => r.data.tags) });
  const [editing, setEditing] = useState(null);

  const mut = useMutation({
    mutationFn: ({ id, ...d }) => adminApi.updateTag(id, d),
    onSuccess: () => { toast.success('Tag updated'); qc.invalidateQueries(['tags']); setEditing(null); },
                ...
                : tags.map((tag) =>
                  editing?.id === tag.id ? (
                    <tr key={tag.id} className="border-b border-hydro-border bg-brand-900/10">
                      ...
                      {['display_name','unit','alarm_low','alarm_high','warn_low','warn_high'].map((k) => (
                        <td key={k} className="px-2 py-1">
                          <input
                            type={['alarm_low','alarm_high','warn_low','warn_high'].includes(k) ? 'number' : 'text'}
                            value={editing[k] ?? ''}
                      ...
                      <td className="px-3 py-2 flex gap-1">
                        <button onClick={() => mut.mutate(editing)} className="text-xs px-2 py-1 rounded bg-emerald-700 text-white">Save</button>
                        <button onClick={() => setEditing(null)} className="text-xs px-2 py-1 rounded bg-hydro-border text-white">Cancel</button>
                      </td>
                    </tr>
                  ) : (
                    <tr key={tag.id} className="table-row">
                      ...
                      <td className="px-3 py-2">
                        <button onClick={() => setEditing({ ...tag })} className="text-xs px-2 py-1 rounded bg-brand-700 text-white">Edit</button>
                      </td>
                    </tr>
                  )
                )
...
// ─── User Settings ────────────────────────────────────────────────────────────
function UserSettings({ qc }) {
  const { data } = useQuery({ queryKey: ['users'], queryFn: () => authApi.listUsers().then((r) => r.data.users) });
  const [showCreate, setShowCreate] = useState(false);
  const [form, setForm] = useState({ name:'', email:'', password:'', role_id: 2 });

  const createMut = useMutation({
    mutationFn: (d) => authApi.createUser(d),
    onSuccess: () => { toast.success('User created'); qc.invalidateQueries(['users']); setShowCreate(false); setForm({ name:'',email:'',password:'',role_id:2 }); },
          ...
          <tbody>
            {(data || []).map((u) => (
              <tr key={u.id} className="table-row text-xs">
                ...
                <td className="px-4 py-3"><span className={u.is_active ? 'badge-ok' : 'badge-crit'}>{u.is_active ? 'Active' : 'Disabled'}</span></td>
                <td className="px-4 py-3 text-slate-500">{u.last_login_at ? new Date(u.last_login_at).toLocaleDateString() : 'Never'}</td>
                <td className="px-4 py-3">
                  <button onClick={() => toggleMut.mutate({ id: u.id, is_active: !u.is_active })} className="text-xs px-2 py-1 rounded bg-hydro-border hover:bg-slate-700 text-white">
...
function NotificationSettings({ qc }) {
  const { data } = useQuery({ queryKey: ['notifications'], queryFn: () => adminApi.notifications().then((r) => r.data.settings) });

  const mut = useMutation({
    mutationFn: ({ id, ...d }) => adminApi.updateNotification(id, d),
    onSuccess: () => { toast.success('Saved'); qc.invalidateQueries(['notifications']); },
  });
        ...
        <div key={setting.id} className="card space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`text-xs px-3 py-1 rounded-full transition-colors ${setting.is_active ? 'bg-emerald-700 text-white' : 'bg-hydro-border text-slate-400'}`}
            >
    ...
    <div className="space-y-4 max-w-lg">
      ...
      <div className="card space-y-3">
        ...
        <div className="flex gap-3">
          <button onClick={() => triggerSummary.mutate()} disabled={triggerSummary.isPending} className="btn-primary text-sm">
