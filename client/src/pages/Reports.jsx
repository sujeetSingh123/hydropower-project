...
export default function Reports() {
  ...
  const handleDownload = async (type, fmt) => {
    ...
    try {
      let resp;
      if (type === 'daily') {
        resp = await reportsApi.downloadDaily(selectedDate, fmt);
        const ext = fmt === 'pdf' ? 'pdf' : 'xlsx';
        downloadBlob(new Blob([resp.data]), `daily_report_${selectedDate}.${ext}`);
      } else {
        resp = await reportsApi.downloadMonthly(selectedYear, selectedMonth, fmt);
      ...
      <div className="flex gap-2 border-b border-hydro-border pb-3">
        {['daily', 'monthly'].map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-lg text-sm font-medium capitalize transition-colors ${tab === t ? 'bg-brand-600 text-white' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
            {t} Report
          </button>
        ))}
        ...
        <div className="space-y-4">
          <div className="card flex flex-wrap items-end gap-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => handleDownload('daily','excel')} disabled={!!downloading} className="btn-primary text-sm">
                {downloading === 'daily-excel' ? 'Downloading...' : '⬇ Excel'}
              </button>
              <button onClick={() => handleDownload('daily','pdf')} disabled={!!downloading} className="btn-ghost text-sm">
              ...
              {[
                ...
                <div key={item.label} className="card text-center">
                  ...
                  <p className="text-2xl font-bold font-mono text-white mt-2">
                    {item.value != null ? Number(item.value).toFixed(item.unit === 'kWh' ? 0 : 2) : '—'}
                  </p>
        ...
        <div className="space-y-4">
          <div className="card flex flex-wrap items-end gap-4">
            ...
            <div>
              ...
              <select value={selectedMonth} onChange={(e) => setSelectedMonth(+e.target.value)} className="input w-32">
                {MONTHS.map((m, i) => <option key={i} value={i + 1}>{m}</option>)}
              </select>
            </div>
            <button onClick={() => handleDownload('monthly','excel')} disabled={!!downloading} className="btn-primary text-sm">
          ...
          {monthlyData?.length > 0 ? (
            <>
              <GenerationChart data={monthlyData} title={`Monthly Generation — ${MONTHS[selectedMonth - 1]} ${selectedYear}`} />
