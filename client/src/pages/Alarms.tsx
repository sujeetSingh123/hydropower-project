...
import type { AlarmSeverity, AlarmStatus, AlarmStats, PaginatedResponse, Alarm } from '../types';
...
export default function Alarms() {
  ...
  const [filters, setFilters] = useState<AlarmFilters>({ status: '', severity: '' });
  const [page, setPage] = useState<number>(0);
  const LIMIT = 25;

  const { data, isLoading } = useQuery<PaginatedResponse<Alarm>>({
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="card flex flex-wrap gap-4 items-center">
        ...
        <button
          onClick={() => { setFilters({ status: '', severity: '' }); setPage(0); }}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center text-slate-500">No alarms found</td>
                </tr>
              ) : (
                alarms.map((a) => (
                  <tr key={a.id} className="table-row">
                    ...
                    <td className="px-4 py-3">
                      <span className={STATUS_CLASS[a.status] ?? 'badge-info'}>{a.status}</span>
                    </td>
