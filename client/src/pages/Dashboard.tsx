...
import type { LiveReadingsMap, LiveReading, DailyGenerationSummary } from '../types';

interface ChartTagConfig {
  tagName:   string;
  label:     string;
  unit:      string;
  color:     string;
...
const CHART_TAGS: ChartTagConfig[] = [
...
function kpiStatus(
  ...
  if ((alarmHigh != null && value > alarmHigh) || (alarmLow != null && value < alarmLow)) return 'critical';
  if ((warnHigh  != null && value > warnHigh)  || (warnLow  != null && value < warnLow))  return 'warning';
  return 'ok';
}

export default function Dashboard() {
  const [liveMap, setLiveMap] = useState<LiveReadingsMap>({});
  const historyRef = useRef<Record<string, HistoryPoint[]>>({});

  // Initial REST fetch for live data
  const { data: initialLive } = useQuery<LiveReadingsMap>({
  ...
  const { data: summaries } = useQuery<DailyGenerationSummary[]>({
  ...
  const handleLiveData = useCallback(({ data }: { data: LiveReadingsMap }) => {
    setLiveMap((prev) => ({ ...prev, ...data }));
    // Append to rolling history
    Object.entries(data).forEach(([tag, d]: [string, LiveReading]) => {
  ...
  const live: LiveReadingsMap = { ...(initialLive ?? {}), ...liveMap };

  const get = (tagName: string): number | null => live[tagName]?.value ?? null;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="card col-span-1 flex flex-col justify-center items-center py-6">
          ...
          <p className="text-4xl font-bold font-mono text-brand-400 mt-2">
            {energy != null ? Number(energy).toLocaleString('en-IN') : '—'}
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {CHART_TAGS.map((cfg) => (
          <LiveChart
            ...
            label={cfg.label}
            unit={cfg.unit}
            color={cfg.color as 'blue' | 'green' | 'amber' | 'red' | 'purple' | 'cyan'}
